...
(() => {
  ...
  /* ── Scenes — using caption property for step navigation ── */
  ...
  function buildScenes() {
    return [
      ...
      {
        time: 0,
        caption: () => NovaMed.Timeline.setCap('The Medical Codes section \u2014 ready for input', 'receipt_long', 'cap-icon-default'),
        action: () => {
          anime({
            targets: sectionEl,
            opacity: [0, 1],
      ...
      {
        time: 5000,
        caption: () => NovaMed.Timeline.setCap('AI generates billing codes from the completed report', 'auto_awesome', 'cap-icon-ai'),
        action: () => {
          ...
          setTimeout(() => {
            ...
            anime({ targets: content, opacity: [0, 1], duration: NORMAL, easing: EASE_IN });
            ...
            setTimeout(() => {
              ...
              CPT_CODES.forEach(c => {
                ...
                anime({ targets: row, opacity: [0, 1], translateX: [-8, 0], duration: NORMAL, easing: EASE_IN });
              });
              ...
              ICD_CODES.forEach((c, i) => {
                ...
                anime({ targets: row, opacity: [0, 1], translateX: [-8, 0], duration: NORMAL, delay: STAGGER + i * STAGGER, easing: EASE_IN });
              });

              setTimeout(() => {
                show(addRow, true);
                anime({ targets: addRow, opacity: [0, 1], duration: NORMAL, easing: EASE_IN });
      ...
      {
        time: 14000,
        caption: () => NovaMed.Timeline.setCap('The physician reviews each code against clinical documentation', 'fact_check', 'cap-icon-physician'),
        action: () => {
      ...
      {
        time: 20000,
        caption: () => NovaMed.Timeline.setCap('Any code can be corrected with inline editing', 'edit', 'cap-icon-physician'),
        action: () => {
          ...
          setTimeout(() => {
            ...
            setTimeout(() => {
              ...
              const codeIv = setInterval(() => {
                if (ci < CORRECTED_CODE.length) {
                  codeEl.insertBefore(document.createTextNode(CORRECTED_CODE[ci]), cur);
                  ci++;
                } else {
      ...
      {
        time: 30000,
        caption: () => NovaMed.Timeline.setCap('Manual codes added alongside AI-generated ones', 'add_circle', 'cap-icon-default'),
        action: () => {
          const addRow = $('billingAddRow');
            ...
            const partIv = setInterval(() => {
              if (pi < partialText.length) {
                ...
                setTimeout(() => {
                  ...
                  $('billingDiagnosisList').appendChild(newRow);
                  addRow.classList.remove('active');
                  textEl.textContent = '+ Write something or hit \'/\'';
                  anime({ targets: newRow, opacity: [0, 1], translateX: [-6, 0], duration: NORMAL, easing: EASE_IN });
                }, 1000);
              }
            }, STAGGER);
          }, 700);
      ...
      // Scene 6: Source distinction (38-42s)
      {
        time: 38000,
        caption: () => NovaMed.Timeline.setCap('AI-generated codes shown in blue \u2014 manually added codes are distinct', 'compare', 'cap-icon-ai'),
        action: () => {
      ...
      // Scene 7: Verification (42-46s)
      {
        time: 42000,
        caption: () => NovaMed.Timeline.setCap('The physician approves all billing codes \u2014 AI-assisted, physician-verified', 'verified_user', 'cap-icon-physician'),
        action: () => {
          const allRows = sectionEl.querySelectorAll('.billing-row');
          allRows.forEach(row => row.classList.remove('active', 'highlight'));
          const addRow = $('billingAddRow');
          ...
          anime({
            targets: sectionEl,
            boxShadow: ['0 1px 2px rgba(0,0,0,0.04)', '0 2px 12px rgba(74,68,89,0.18)', '0 1px 2px rgba(0,0,0,0.04)'],
            duration: 1500,
            easing: EASE_OUT
          });
      ...
      // Scene 8: Stats (46-50s)
      {
        time: 46000,
        caption: () => NovaMed.Timeline.setCap('Complete billing codes \u2014 AI-generated and physician-verified', 'timer', 'cap-icon-default'),
        action: () => {
